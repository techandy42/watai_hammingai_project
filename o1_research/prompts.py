...
class O1BaselinePrompts:
    ...
    @staticmethod
    def get_rank_internal_question_prompt(thought_chain: Thought) -> str:
        prompt = f"""
        Instruction:
        \"\"\"
        ...
        Current Questions:
        \"\"\"
        - a: {thought_chain.chain[-1].questions[0]}
        - b: {thought_chain.chain[-1].questions[1]}
        - c: {thought_chain.chain[-1].questions[2]}
        - d: {thought_chain.chain[-1].questions[3]}
        \"\"\"
        """
    ...
    @staticmethod
    def get_rank_external_question_prompt(thought_chain: Thought) -> str:
        prompt = f"""
        Instruction:
        \"\"\"
        - The following are the initial problem statement, the previous steps of reasoning, and the current questions.
        - The current questions that ask to solve the initial problem statement using the previous steps of reasoning are listed below with labels: a, b, c, d
        - Return the label of the current question that you think is the most adequate to solve the initial problem statement.
        - "choice": "a", "b", "c", or "d" (only one of the following labels)
        \"\"\"
...
if __name__ == "__main__":
    ...
    print("\nTest 2: Question Prompt with One Thought")
    print(question_prompt_2)

    # Test 3: get_followup_question_prompt with two thoughts
    thought_chain_3 = ThoughtChain(initial_question="How can I optimize matrix multiplication?")
    thought_2 = Thought()
    ...
    print("\nTest 3: Question Prompt with Two Thoughts")
    print(question_prompt_3)

    # Test 4: get_internal_answer_prompt
    internal_answer_prompt_1 = O1BaselinePrompts.get_internal_answer_prompt(thought_chain_3)
    print("\nTest 4: Internal Answer Prompt")
    print(internal_answer_prompt_1)

    # Test 5: get_external_answer_prompt
    external_answer_prompt_1 = O1BaselinePrompts.get_external_answer_prompt(thought_chain_3)
    print("\nTest 5: External Answer Prompt")
    print(external_answer_prompt_1)

    # Test 6: get_rank_internal_question_prompt
    ...
    rank_internal_question_prompt = O1BaselinePrompts.get_rank_internal_question_prompt(thought_chain_3)
    print("\nTest 6: Rank Internal Question Prompt")
    print(rank_internal_question_prompt)

    # Test 7: get_rank_external_question_prompt
    rank_external_question_prompt = O1BaselinePrompts.get_rank_external_question_prompt(thought_chain_3)
    print("\nTest 7: Rank External Question Prompt")
    print(rank_external_question_prompt)

    # Test 8: get_rank_answer_prompt
